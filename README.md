...
|---|---|---|
| Terraform module | ✅ `terraform/main.tf` — VPC/SG/ECS **and now EVS/EIP**, all **verified against a real Huawei Cloud sandbox** (apply+destroy), validated with `terraform validate`/`tflint` in CI | Remote state backend, Day-2 ops beyond create/destroy, the GitHub Actions smoke-test workflow itself is still untriggered |
