...
| **HC ITOM Connector productization** | ✅ **Phase 2A + 2B (of 6), both real-PDI verified**: multi-account/region ECS orchestrator (`HcConnectorEcsSync.js`, Phase 2A) — HC2/HC3/HC4 directly exercised, HC1/HC5 on lighter evidence; VPC + Subnet discovery (`HuaweiVpcDiscovery.js`/`HcConnectorVpcSync.js`, Phase 2B) — HC6–HC10 all directly exercised. Scope narrowed from the original VPC/Subnet/EVS/EIP wording — EVS/EIP discovery deferred to Phase 2C; its Terraform grounding is now real-verified (apply+destroy against the real sandbox). Real codegen covers both orchestrators via one manifest-driven build script. See [servicenow/hc-connector/README.md](servicenow/hc-connector/README.md), [ARCHITECTURE.md](servicenow/hc-connector/docs/ARCHITECTURE.md) | Phase 2C (EVS/EIP Discovery), Phase 2D (Security Group Discovery); Phases 3–6: platform services (ELB/RDS/OBS/CCE), opt-in Pod discovery, a real event gateway, CPG/Terraform catalog + Day-2 ops |
