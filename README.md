...
|---|---|---|
| Terraform module | ✅ `terraform/main.tf` — VPC/SG/ECS/EVS/EIP/ELB/RDS/OBS, all **verified against a real Huawei Cloud sandbox** (apply+destroy), validated with `terraform validate`/`tflint` in CI. CCE deliberately not attempted yet (different order of magnitude in cost/creation time) | Remote state backend, Day-2 ops beyond create/destroy, the GitHub Actions smoke-test workflow itself is still untriggered, CCE |
