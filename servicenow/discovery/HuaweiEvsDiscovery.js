...
// precedent) - covered by check-mirror-drift.js's fourth PAIRS entry.
        ...
        for (var attempt = 0; attempt <= this.maxRetries; attempt++) {
            try {
                ...
                if (this._shouldRetry(status, attempt)) {
                    ...
                    // retryable error.
                    gs.warn('[HuaweiEvsDiscovery] fetch got ' + status + ' at offset ' + offset + ', retrying immediately (no backoff, attempt ' + attempt + ')');
                    continue;
                }
