// Script Include: HuaweiElbDiscovery
// HC ITOM Connector Phase 3 - ELB (Elastic Load Balance, dedicated/v3)
// discovery. New sibling to HuaweiECSDiscovery.js/HuaweiVpcDiscovery.js/
// HuaweiEvsDiscovery.js, not a modification of any of them - all three
// are real-PDI verified and intentionally left untouched.
//
// ELB uses its own API family (elb.{region}.myhuaweicloud.com, real-PDI
// confirmed), not ECS's/VPC's/EVS's host, so this is a new file rather
...
// precedent) - covered by check-mirror-drift.js's fifth PAIRS entry.
// _computeBackoffMs was removed - gs.sleep() is fenced for custom scoped
// apps on this instance, so backoff delay isn't usable here; retries now
// happen immediately instead.
//
// Pagination: GET /v3/{project_id}/elb/loadbalancers uses the same
// marker/cursor shape as VPC/Subnet/Security Group (page_info.next_marker,
// no total count field), real-PDI confirmed. Reuses the exact same
// _shouldFetchNextPage() logic as HuaweiVpcDiscovery.js (mirrors
// lib/vpcPagination.js's shouldFetchNextPage(), duplicated here rather
// than shared since ServiceNow scoped scripts cannot require()).
//
// CI class (CI_CLASS_ELB below) is `cmdb_ci_cloud_load_balancer`,
...
// - the same pattern already proven for VPC/Security Group/EVS, NOT the
// `vpc_id`-based relation that field's real presence in the payload might
// have suggested (this project's established process - let the real
// error decide rather than guess ahead of it - paid off again here). Each
// load balancer relates to a local cloud_service_account/
// logical_datacenter placeholder pair, same pattern as EVS. See
// lib/mapElbToIRE.js's header comment for the full reasoning.
//
...
// HcConnectorElbSync's config) - reuse the same already-validated AK/SK,
// nothing new to provision.
var HuaweiElbDiscovery = Class.create();
HuaweiElbDiscovery.prototype = {
    CI_CLASS_ELB: 'cmdb_ci_cloud_load_balancer',
    ...
    // ------------------------------------------------------------------
    // Fetch load balancers, paginated, with retry (AK/SK signed, no auth call)
    // ------------------------------------------------------------------
    ...
    fetchLoadBalancers: function() {
        var allLoadBalancers = [];
        var marker = null;

        while (true) {
            var page = this._fetchPage(marker);

            if (page === null) {
                throw new Error('ELB fetch failed' + (marker ? (' (marker=' + marker + ')') : '') +
                    ' after retries - see the prior gs.error log for the underlying status/exception');
            }

            allLoadBalancers = allLoadBalancers.concat(page.loadBalancers);

            var continuePaging = this._shouldFetchNextPage({
                pageItemCount: page.loadBalancers.length,
    ...
    _fetchPage: function(marker) {
        var host = 'elb.' + this.region + '.myhuaweicloud.com';
        var pathname = '/v3/' + this.projectId + '/elb/loadbalancers';
        ...
        for (var attempt = 0; attempt <= this.maxRetries; attempt++) {
            try {
                ...
                if (status == 200) {
                    ...
                    return { loadBalancers: body.loadbalancers || [], nextMarker: pageInfo.next_marker || null };
                }
                ...
                if (this._shouldRetry(status, attempt)) {
                    ...
                    // retryable error.
                    gs.warn('[HuaweiElbDiscovery] fetch got ' + status + ', retrying immediately (no backoff, attempt ' + attempt + ')');
                    continue;
                }

                gs.error('[HuaweiElbDiscovery] fetch failed: ' + status + ' - ' + response.getBody());
                return null;
            } catch (ex) {
                gs.error('[HuaweiElbDiscovery] fetch exception: ' + ex.message);
                return null;
            }
        }
        return null;
    ...
    reconcileCIs: function(loadBalancers) {
        ...
        try {
            ...
            gs.info('[HuaweiElbDiscovery] IRE result for ' + loadBalancers.length + ' load balancer(s): ' + rawResult);
            return JSON.parse(rawResult);
        } catch (ex) {
            gs.error('[HuaweiElbDiscovery] IRE exception: ' + ex.message);
        }
    },

    run: function() {
        var loadBalancers = this.fetchLoadBalancers();
        gs.info('[HuaweiElbDiscovery] Fetched ' + loadBalancers.length + ' load balancer(s) across all pages');
        if (loadBalancers.length) this.reconcileCIs(loadBalancers);
    },
    ...
    // check-mirror-drift.js's fifth PAIRS entry.
    // ------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------
    // Pagination - marker-based, mirrors lib/vpcPagination.js's
    // shouldFetchNextPage() (same shape as HuaweiVpcDiscovery.js's
    // _shouldFetchNextPage()).
    // ------------------------------------------------------------------
