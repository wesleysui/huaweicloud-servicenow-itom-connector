...
| EVS (disk) | ✅ real-PDI verified (`terraform/main.tf`, `huaweicloud_evs_volume` + `huaweicloud_compute_volume_attach`, apply+destroy against real sandbox) | 🚧 source-complete, not yet real-PDI verified — new `HuaweiEvsDiscovery.js`/`HcConnectorEvsSync.js`, CI class `cmdb_ci_storage_volume` (from AWS SGC docs, unconfirmed here), relates each volume to its ECS instance via a REAL sys_id lookup (not an array index) - the first attempt at a cross-discovery-run relation in this project, unconfirmed whether IRE accepts it | 🚧 Cloud Eye emits EVS alarms; no ingestion mapping yet | ❌ | Phase 2C — Terraform done, Discovery source-complete, real-PDI pending |
