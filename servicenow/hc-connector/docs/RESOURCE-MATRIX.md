...
| EVS (disk) | ✅ real-PDI verified (`terraform/main.tf`, `huaweicloud_evs_volume` + `huaweicloud_compute_volume_attach`, apply+destroy against real sandbox) | ✅ real-PDI verified — `HuaweiEvsDiscovery.js`/`HcConnectorEvsSync.js` fetch/reconcile EVS volumes into `cmdb_ci_storage_volume` (CI class from AWS SGC research, real-PDI confirmed with a working OOTB Identification Rule), related to the shared logical-datacenter placeholder via `Hosted on::Hosts`. No relation to ECS — a real cross-discovery-run relation attempt (passing the ECS CI's committed sys_id as a `relations[].child` value) was tried and definitively disproven: ServiceNow's server-side parser deserializes that field as a Java `Integer`, confirmed via a real `InvalidFormatException` naming the exact field, so it can only ever hold an array index, never a real sys_id — same finding as Security Group's ECS relation. Idempotent on a second run (`insertCount:0, refreshCount:1`, all `NO_CHANGE`) | 🚧 Cloud Eye emits EVS alarms; no ingestion mapping yet | ❌ | Phase 2C — Terraform + Discovery both real-PDI verified |
| EIP | ✅ real-PDI verified (`terraform/main.tf`, `huaweicloud_vpc_eip` + `huaweicloud_vpc_eip_associate`, apply+destroy against real sandbox) | ✅ real-PDI verified — `HuaweiVpcDiscovery.js`/`HcConnectorVpcSync.js` fetch/reconcile EIPs into `cmdb_ci_ip_address` (CI class from AWS SGC research). Its real OOTB containment rule is different from every other resource here (confirmed via a real `MISSING_DEPENDENCY` error): needs an `Owns::Owned by` relation to a `cmdb_ci_hardware`-family CI, not `Hosted on::Hosts` to a datacenter. ECS's own class (`cmdb_ci_vm_instance`) was checked via a real `sys_db_object` class-hierarchy walk and confirmed NOT hardware-family, ruling that out; `cmdb_ci_virtualization_server` (the shared placeholder `HuaweiECSDiscovery.js` already creates) was confirmed hardware-family, so each EIP relates to a freshly-built local stub of that same placeholder — IRE matches it against the real, already-committed CI from the separate ECS discovery run via identification matching (same mechanism AWS's connector uses for cross-payload relationships), not a raw sys_id. Real-PDI verified end to end (`hasError:false`, EIP + relation both inserted, the stub correctly matched as `NO_CHANGE` not a duplicate) and idempotent on a second run (`insertCount:0, refreshCount:1`, all `NO_CHANGE`, no warnings) | ❌ | ❌ | Phase 2C — Terraform + Discovery both real-PDI verified |
| Security Group | ✅ verified (part of `terraform/main.tf`, `huaweicloud_networking_secgroup` + rule, already used since Phase 1) | ✅ real-PDI verified — `HuaweiVpcDiscovery.js`/`HcConnectorVpcSync.js` fetch/reconcile Security Groups into `cmdb_ci_compute_security_group` (CI class from AWS SGC research, real-PDI confirmed with a working OOTB Identification Rule), related to the shared logical-datacenter placeholder via `Hosted on::Hosts` (NOT to VPC - real API has no `vpc_id`, and the real OOTB containment rule wants a datacenter parent, confirmed via a real MISSING_DEPENDENCY error); idempotent on a second run. No relation to ECS (cross-discovery-run relations aren't a solved pattern here) | — | ❌ | Phase 2C — Terraform + Discovery both real-PDI verified |
