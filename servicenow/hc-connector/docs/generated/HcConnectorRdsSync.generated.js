...
// Script Include: HcConnectorRdsSync
...
// producing docs/generated/HcConnectorRdsSync.generated.js - paste THAT.
//
// Multi-account/region RDS sync orchestrator (HC ITOM Connector Phase 3).
// Sibling to HcConnectorEcsSync.js/HcConnectorVpcSync.js/HcConnectorEvsSync.js/
// HcConnectorElbSync.js, not a refactor of any of them - all four are
// real-PDI verified and intentionally left untouched.
//
// No relations attempted yet (see
// servicenow/discovery/HuaweiRdsDiscovery.js's header comment - the real
// OOTB containment rule isn't known until the first real MISSING_DEPENDENCY
// error). Structure is otherwise identical to HcConnectorElbSync.js: for
...
var HcConnectorRdsSync = Class.create();
HcConnectorRdsSync.prototype = {
    initialize: function() {
    ...
    runAll: function() {
        ...
        gs.info('[HcConnectorRdsSync] starting run across ' + accounts.length + ' active account(s)');
        accounts.forEach(function(account) {
            ...
            regions.forEach(function(region) {
                try {
                    ...
                    gs.error('[HcConnectorRdsSync] unexpected failure for account=' + account.account_id +
                        ' region=' + region.region + ': ' + ex.message);
                    self._recordRegionError(region.sys_id, ex.message);
                }
            });
        });
        gs.info('[HcConnectorRdsSync] run complete');
    },
    ...
    _runForAccountRegion: function(account, region) {
        ...
        var fetchResult;
        try {
            fetchResult = this._fetchInstances(account, region);
        } catch (fetchEx) {
            gs.error('[HcConnectorRdsSync] fetch failed for account=' + account.account_id +
        ...
        var reconcileResult;
        try {
            reconcileResult = this._reconcileAndUpsert(account, region, fetchResult.instances);
        } catch (reconcileEx) {
            gs.error('[HcConnectorRdsSync] reconcile failed for account=' + account.account_id +
                ' region=' + region.region + ': ' + reconcileEx.message);
            this._recordRegionError(region.sys_id, reconcileEx.message);
            this._updateDiscoveryRun(runRecordSysId, finishRun(runFields,
                { successCount: 0, failCount: fetchResult.instances.length, errorSummary: reconcileEx.message, endedAtMs: new Date().getTime() }));
        ...
        gs.info('[HcConnectorRdsSync] account=' + account.account_id + ' region=' + region.region +
            ' done: ' + JSON.stringify(reconcileResult.summary));
    },

    // ------------------------------------------------------------------
    // Fetch (delegates to HuaweiRdsDiscovery)
    // ------------------------------------------------------------------
    _fetchInstances: function(account, region) {
        ...
        var disco = new HuaweiRdsDiscovery({
            region: region.region,
            projectId: region.project_id,
            pageLimit: this._getConnectorConfigValue('rds_page_limit', 100),
            maxRetryAttempts: this._getConnectorConfigValue('rds_max_retry_attempts', 3),
            accessKey: credentials.accessKey,
            secretKey: credentials.secretKey
        });

        var instances = disco.fetchInstances(); // throws on a page failure - see HuaweiRdsDiscovery.js
        return { disco: disco, instances: instances };
    },

    // ------------------------------------------------------------------
    // Reconcile (delegates to HuaweiRdsDiscovery.reconcileCIs)
    // + upsert HC Resource Sync State + retirement
    // ------------------------------------------------------------------
    _reconcileAndUpsert: function(account, region, instances) {
        var self = this;

        var disco = new HuaweiRdsDiscovery({ region: region.region, accountId: account.account_id });
        var result = disco.reconcileCIs(instances);

        if (!result) {
            throw new Error('reconcileCIs returned no result - the IRE call likely failed, see the prior gs.error log from HuaweiRdsDiscovery');
        }
        if (result.hasError) {
        ...
        var seenRecords = instances.map(function(instance) {
            return { native_key: instance.id, ci: self._lookupCiByCorrelationId(instance.id) };
        });
    ...
    _lookupCiByCorrelationId: function(correlationId) {
        var gr = new GlideRecord('cmdb_ci_cloud_database');
    ...
    _retireCi: function(ciSysId) {
        ...
        if (retiredValue == null) {
            gs.warn('[HcConnectorRdsSync] could not resolve cmdb_ci.install_status "Retired" choice value - CI ' +
        ...
        gr.update();
        gs.info('[HcConnectorRdsSync] retired CI ' + ciSysId);
    },
