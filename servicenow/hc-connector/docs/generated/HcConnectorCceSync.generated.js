...
// Script Include: HcConnectorCceSync
...
// producing docs/generated/HcConnectorCceSync.generated.js - paste THAT.
//
// Multi-account/region CCE CLUSTER sync orchestrator (HC ITOM Connector
// Phase 3) - cluster only, see servicenow/discovery/HuaweiCceDiscovery.js's
// header comment for why node/namespace/workload/service/ingress are out
// of scope. Sibling to HcConnectorEcsSync.js/HcConnectorVpcSync.js/
// HcConnectorEvsSync.js/HcConnectorElbSync.js/HcConnectorRdsSync.js/
// HcConnectorObsSync.js, not a refactor of any of them - all six are
// real-PDI verified and intentionally left untouched.
//
// No relations attempted yet (see HuaweiCceDiscovery.js's header comment
// - the real OOTB containment rule isn't known until the first real
// MISSING_DEPENDENCY error, and a brand-new custom class may not even
// have one at all, matching OBS's own outcome). Structure is otherwise
// identical to HcConnectorObsSync.js: for every active HC Cloud Region
// under every active HC Cloud Account, resolve credentials, fetch,
// reconcile via IRE, upsert HC Resource Sync State, retire on
...
var HcConnectorCceSync = Class.create();
HcConnectorCceSync.prototype = {
    initialize: function() {
        ...
        this.RESOURCE_TYPE = 'cce_cluster';
    },
    ...
    runAll: function() {
        ...
        gs.info('[HcConnectorCceSync] starting run across ' + accounts.length + ' active account(s)');
        accounts.forEach(function(account) {
            ...
            regions.forEach(function(region) {
                try {
                    ...
                    gs.error('[HcConnectorCceSync] unexpected failure for account=' + account.account_id +
                        ' region=' + region.region + ': ' + ex.message);
                    self._recordRegionError(region.sys_id, ex.message);
                }
            });
        });
        gs.info('[HcConnectorCceSync] run complete');
    },
    ...
    _runForAccountRegion: function(account, region) {
        ...
        var fetchResult;
        try {
            fetchResult = this._fetchClusters(account, region);
        } catch (fetchEx) {
            gs.error('[HcConnectorCceSync] fetch failed for account=' + account.account_id +
        ...
        var reconcileResult;
        try {
            reconcileResult = this._reconcileAndUpsert(account, region, fetchResult.clusters);
        } catch (reconcileEx) {
            gs.error('[HcConnectorCceSync] reconcile failed for account=' + account.account_id +
                ' region=' + region.region + ': ' + reconcileEx.message);
            this._recordRegionError(region.sys_id, reconcileEx.message);
            this._updateDiscoveryRun(runRecordSysId, finishRun(runFields,
                { successCount: 0, failCount: fetchResult.clusters.length, errorSummary: reconcileEx.message, endedAtMs: new Date().getTime() }));
        ...
        gs.info('[HcConnectorCceSync] account=' + account.account_id + ' region=' + region.region +
            ' done: ' + JSON.stringify(reconcileResult.summary));
    },

    // ------------------------------------------------------------------
    // Fetch (delegates to HuaweiCceDiscovery)
    // ------------------------------------------------------------------
    _fetchClusters: function(account, region) {
        ...
        var disco = new HuaweiCceDiscovery({
            region: region.region,
            projectId: region.project_id,
            maxRetryAttempts: this._getConnectorConfigValue('cce_max_retry_attempts', 3),
            accessKey: credentials.accessKey,
            secretKey: credentials.secretKey
        });

        var clusters = disco.fetchClusters(); // throws on a fetch failure - see HuaweiCceDiscovery.js
        return { disco: disco, clusters: clusters };
    },

    // ------------------------------------------------------------------
    // Reconcile (delegates to HuaweiCceDiscovery.reconcileCIs)
    // + upsert HC Resource Sync State + retirement
    // ------------------------------------------------------------------
    _reconcileAndUpsert: function(account, region, clusters) {
        var self = this;

        var disco = new HuaweiCceDiscovery({ region: region.region, accountId: account.account_id });
        var result = disco.reconcileCIs(clusters);

        if (!result) {
            throw new Error('reconcileCIs returned no result - the IRE call likely failed, see the prior gs.error log from HuaweiCceDiscovery');
        }
        if (result.hasError) {
        ...
        var seenRecords = clusters.map(function(cluster) {
            var uid = (cluster.metadata || {}).uid;
            return { native_key: uid, ci: self._lookupCiByCorrelationId(uid) };
        });
    ...
    _lookupCiByCorrelationId: function(correlationId) {
        var gr = new GlideRecord('x_2021019_huawei_0_huawei_cloud_cce_cluster');
    ...
    _retireCi: function(ciSysId) {
        ...
        if (retiredValue == null) {
            gs.warn('[HcConnectorCceSync] could not resolve cmdb_ci.install_status "Retired" choice value - CI ' +
        ...
        gr.update();
        gs.info('[HcConnectorCceSync] retired CI ' + ciSysId);
    },
