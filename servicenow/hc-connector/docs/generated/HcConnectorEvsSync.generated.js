...
// + lib/credentialProvider.js, lib/resourceLifecycle.js, lib/compositeKey.js, lib/discoveryRunTracker.js, lib/syncStatePlanner.js, lib/diskAggregation.js
...
// lib/syncStatePlanner.js, and lib/diskAggregation.js (module.exports
// stripped) at the marker below, producing
// docs/generated/HcConnectorEvsSync.generated.js - paste THAT.
//
...
// untouched.
//
// No CMDB relation to ECS is attempted (see
...
//
// CI hardware fields side effect (added after ECS's own cpus/memory/nics
// work - see HuaweiECSDiscovery.js): since a real CMDB relation to ECS is
// off the table (above), this orchestrator instead does a direct
// GlideRecord field update on the matching cmdb_ci_vm_instance CI's
// disks/disks_size, using the SAME EVS volume list already fetched for
// reconciliation - no new Huawei API call. See _updateEcsDiskFields()
// below and lib/diskAggregation.js's pure aggregation logic. Soft ordering
// dependency: only updates CIs that already exist (correlation_id lookup)
// - if EVS syncs before an ECS CI has ever been created, that instance's
// disks fields just stay empty until a later EVS run catches up, same
// self-healing-on-repeat-runs philosophy as the rest of this project.
...
// ---- inlined from lib/diskAggregation.js (do not hand-edit here - edit the source and regenerate) ----
/**
...
HcConnectorEvsSync.prototype = {
    ...
    _reconcileAndUpsert: function(account, region, volumes) {
        ...
        this._updateEcsDiskFields(volumes);
    ...
    // ------------------------------------------------------------------
    // CI hardware fields side effect - see this file's header comment for
    // why this is a direct field update rather than a CMDB relation.
    // aggregateDisksByServer() (inlined pure lib) groups this run's already-
    // fetched volumes by attached server_id and sums count/size; this method
    // just does the GlideRecord write for each server_id found. Skips (no-op,
    // not an error) any server_id with no matching cmdb_ci_vm_instance CI yet
    // - self-heals on a later run once ECS Discovery has created it.
    // ------------------------------------------------------------------
    _updateEcsDiskFields: function(volumes) {
        var bySeverId = aggregateDisksByServer(volumes);
        for (var serverId in bySeverId) {
            var gr = new GlideRecord('cmdb_ci_vm_instance');
            gr.addQuery('correlation_id', serverId);
            gr.query();
            if (!gr.next()) continue;
            gr.disks = bySeverId[serverId].count;
            gr.disks_size = bySeverId[serverId].totalSize;
            gr.update();
        }
    },
